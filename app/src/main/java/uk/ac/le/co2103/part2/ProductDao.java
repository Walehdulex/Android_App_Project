...
public interface ProductDao {
    ...
    void deleteAll();
    @Query("SELECT * FROM products WHERE listId = :shoppingListId")
    LiveData<List<Product>> getProductsByShoppingListId(int shoppingListId);
    @Query("SELECT EXISTS(SELECT * FROM products WHERE name = :name AND listId =:shoppingListId)")
    Boolean productExistsInList(String name, int shoppingListId);
    @Query("SELECT EXISTS(SELECT * FROM products WHERE name = :name AND listId =:shoppingListId AND productId =:productId)")
